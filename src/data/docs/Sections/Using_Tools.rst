...
Help command for *sslm-mgr* ::
    ...
    options:
        ...
        --clean-all               Clean up all grps/svcs not just active domains
        --clean-keep   <Num>      Clean database keeping newest Num
        --debug                   debug mode : print but dont do it
        --dns-refresh             dns: Use script to sign zones & restart primary
                                  See config dns.restart_cmd
        --force                   Forces renew / roll / prod check
        --dry-run                 Letsencrypt --dry-run
        --reuse                   Reuse curr key with renew.tlsa unchanged if 
                                  using selector=1 (pubkey)
        --renew                   Renew keys/csr/cert keep in next 
                                  See also config setting renew_expire_days
        --roll                    Roll Phase : Make next be the new curr then
                                   copy to production
                                  Refresh dns if needed
        --min-roll-mins <minutes> Only roll if next is older than this 
                                  See config min_roll_mins.
        --status                  Display cert status. With --verb shows more info
        --test                    Letsencrypt --test-cert
...
To see developer help::
    ...
    options:
        ...
        --new-cert                Make new next/cert
        --copy-csr                Copy curr key to next (used by --reuse)
        --new-csr                 Make next CSR
        --force-server-restarts   Forces server restarts even if not needed
        --new-keys                Make next new keys
        --next-to-curr            Move next to curr
