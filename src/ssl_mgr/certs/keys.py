...
def gen_key_rsa(bits: int = 2048,) -> bytes:
    ...
    try:
        ...
        print(f'Failed to generate RSA key : expon={expon} bits={bits} : {exc}')
        return None

    key_pem =  key.private_bytes(encoding=Encoding.PEM, format=PrivateFormat.PKCS8,
                                 encryption_algorithm=NoEncryption())
    return key_pem

def gen_key_ec(elliptic_curve: str = None) -> bytes:
    """
    ...
    try:
        ...
        print(f'Unsupported elliptic curve: {elliptic_curve} : {exc}') #from exc
        return None

    except UnsupportedAlgorithm as exc:
        print(f'Unsupported Algo : {exc}') #from exc
        return None
